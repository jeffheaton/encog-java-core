 ...
 * 
 */
public class RealTimeAnalystWizard {
	 ...
	 */
	public RealTimeAnalystWizard(final EncogAnalyst theAnalyst) {
	...
	private void generateFilenames(final File rawFile) {
		...
		p.setFilename(RealTimeAnalystWizard.FILE_RAW, this.filenameRaw);
		
		if( this.preprocess ) {
			p.setFilename(RealTimeAnalystWizard.FILE_PRE, this.filenameProcess);
		}
		
		if (this.taskNormalize) {
			p.setFilename(RealTimeAnalystWizard.FILE_NORMALIZE, this.filenameNorm);
		}

		if (this.taskRandomize) {
			p.setFilename(RealTimeAnalystWizard.FILE_RANDOM, this.filenameRandom);
		}

		if (this.taskCluster) {
			p.setFilename(RealTimeAnalystWizard.FILE_CLUSTER, this.filenameCluster);
		}

		if (this.taskSegregate) {
			p.setFilename(RealTimeAnalystWizard.FILE_TRAIN, this.filenameTrain);
			p.setFilename(RealTimeAnalystWizard.FILE_EVAL, this.filenameEval);
			p.setFilename(RealTimeAnalystWizard.FILE_EVAL_NORM, this.filenameEvalNorm);
		}

		if (this.taskBalance) {
			p.setFilename(RealTimeAnalystWizard.FILE_BALANCE, this.filenameBalance);
		}
		
		if (this.codeTargetLanguage != TargetLanguage.NoGeneration ) {
			p.setFilename(RealTimeAnalystWizard.FILE_CODE, this.filenameCode);
		}

		p.setFilename(RealTimeAnalystWizard.FILE_TRAINSET, this.filenameTrainSet);
		p.setFilename(RealTimeAnalystWizard.FILE_ML, this.filenameML);
		p.setFilename(RealTimeAnalystWizard.FILE_OUTPUT, this.filenameOutput);
	...
	private void generateSegregate() {
		if (this.taskSegregate) {
			...
			array[0] = new AnalystSegregateTarget(RealTimeAnalystWizard.FILE_TRAIN,
					DEFAULT_TRAIN_PERCENT);
			array[1] = new AnalystSegregateTarget(RealTimeAnalystWizard.FILE_EVAL,
	...
	private void generateSettings() {
		...
		// starting point
		target = RealTimeAnalystWizard.FILE_RAW;
		this.script.getProperties().setProperty(
		...
		if( this.preprocess ) {
			this.script.getProperties().setProperty(
					...
					target);
			target = RealTimeAnalystWizard.FILE_PRE;
			this.script.getProperties().setProperty(
		...
		if (!this.timeSeries && this.taskRandomize) {
			this.script.getProperties().setProperty(
					...
					target);
			target = RealTimeAnalystWizard.FILE_RANDOM;
			this.script.getProperties().setProperty(
		...
		if (!this.timeSeries && this.taskBalance) {
			...
			target = RealTimeAnalystWizard.FILE_BALANCE;
			this.script.getProperties().setProperty(
		...
		if (this.taskSegregate) {
			...
			target = RealTimeAnalystWizard.FILE_TRAIN;
		}
		...
		if (this.taskNormalize) {
			...
			target = RealTimeAnalystWizard.FILE_NORMALIZE;
			this.script.getProperties().setProperty(
		...
		if (this.taskSegregate) {
			evalSource = RealTimeAnalystWizard.FILE_EVAL;
		} else {
			evalSource = target;
		}
		...
		if (this.taskCluster) {
			...
			this.script.getProperties().setProperty(
					...
					RealTimeAnalystWizard.FILE_CLUSTER);
			this.script.getProperties().setProperty(
		...
		this.script.getProperties().setProperty(
				...
				RealTimeAnalystWizard.FILE_TRAINSET);
		...
		this.script.getProperties().setProperty(
				...
				RealTimeAnalystWizard.FILE_ML);
		this.script.getProperties().setProperty(
				ScriptProperties.ML_CONFIG_OUTPUT_FILE,
				RealTimeAnalystWizard.FILE_OUTPUT);
	...
	private void generateTasks() {
		...
		task3.getLines().add(
				createSet(ScriptProperties.ML_CONFIG_EVAL_FILE,
						RealTimeAnalystWizard.FILE_EVAL_NORM));
		task3.getLines().add(
				createSet(ScriptProperties.NORMALIZE_CONFIG_SOURCE_FILE,
						RealTimeAnalystWizard.FILE_EVAL));
		task3.getLines().add(
			...
			String predictField) 
	{
		...
		this.leadWindowSize = forwardWindow;
		...
		generateProcess(backwardWindow, 
				forwardWindow, 
				prediction, 
				predictField);
	...
	private void generateCode() {
		...
		this.script.getProperties().setProperty(ScriptProperties.CODE_CONFIG_TARGET_FILE, RealTimeAnalystWizard.FILE_CODE);
	}
	...
	private void generateProcess(int backwardWindow, int forwardWindow, PredictionType prediction, String predictField) {
		Map<String,ProcessField> map = new HashMap<String,ProcessField>();
		...
		for(DataField df: this.script.getFields()) {
			...
			if( df.getName().equalsIgnoreCase("time") ) {
				...
				command.append("\",0");
				command.append("))");
				ProcessField pf = new ProcessField(df.getName(), command.toString());
				fields.add(pf);
				map.put(df.getName().toLowerCase(), pf);
			}
		}
		
		// find the predict field
		if( predictField==null ) {
			throw new AnalystError("No prediction field specified.");
		}

		if( map.containsKey(predictField.toLowerCase()) ) {
			StringBuilder cmd = new StringBuilder();
			String n = map.get(predictField.toLowerCase()).getName();
			cmd.append("fieldmax");
			cmd.append("(\"");
			cmd.append(n);
			cmd.append("\",");
			cmd.append(-backwardWindow);
			cmd.append(",");
			cmd.append("-1)");
			
			fields.add(new ProcessField("prediction","fieldmax(\"close\",-5,-1)"));
			this.targetFieldName = "prediction";
		} else {
			throw new AnalystError("Can't find prediction field: " + predictField);
		}
