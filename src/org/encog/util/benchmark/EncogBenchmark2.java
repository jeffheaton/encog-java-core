...
public class EncogBenchmark2 {
	 ...
	 */
	public EncogBenchmark2(final StatusReportable report) {
		this.report = report;
	}
	...
	private double benchmar0Hidden() {
		...
		network.addLayer(new BasicLayer(EncogBenchmark2.INPUT_COUNT));
		network.addLayer(new BasicLayer(EncogBenchmark2.OUTPUT_COUNT));
	...
	private double benchmar1Hidden() {
		final BasicNetwork network = new BasicNetwork();
		network.addLayer(new BasicLayer(EncogBenchmark2.INPUT_COUNT));
		network.addLayer(new BasicLayer(EncogBenchmark2.HIDDEN_COUNT));
		network.addLayer(new BasicLayer(EncogBenchmark2.OUTPUT_COUNT));
		network.getStructure().finalizeStructure();
		(new ConsistentRandomizer(-1,1)).randomize(network);
	...
	public double process() {
		...
		this.report.report(EncogBenchmark2.STEPS, 0, "Beginning benchmark");
		double total = 0;
		total += trainElman();
	...
	private double trainElman() {
		...
		final BasicNetwork network = new BasicNetwork();
		network.addLayer(new BasicLayer(EncogBenchmark2.INPUT_COUNT));
		hidden = new BasicLayer(EncogBenchmark2.HIDDEN_COUNT);
