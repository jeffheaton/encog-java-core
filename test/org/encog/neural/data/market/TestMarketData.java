...
public class TestMarketData extends TestCase {
	...
	public void testMarketData() throws Exception
	{
		...
		marketData.generate();
		TestCase.assertEquals(21,marketData.getPoints().size());
		...
		point = itr.next();
		TestCase.assertEquals(5, point.getSequence());
		TestCase.assertEquals(4, point.getData().length);
		TestCase.assertEquals(175.16,point.getData(0));
		TestCase.assertEquals(26.03,point.getData(1));
		TestCase.assertEquals(29,(int)(point.getData(2)/1000000));
		TestCase.assertEquals(71,(int)(point.getData(3)/1000000));

		// now check the actual data
		TestCase.assertEquals(15,marketData.getData().size());
		...
		TestCase.assertEquals(-0.026, Math.round(pair.getInput().getData(0)*1000.0)/1000.0);
		TestCase.assertEquals(-0.033, Math.round(pair.getInput().getData(1)*1000.0)/1000.0);		
		TestCase.assertEquals(-0.529, Math.round(pair.getInput().getData(2)*1000.0)/1000.0);
		TestCase.assertEquals(-0.63, Math.round(pair.getInput().getData(3)*1000.0)/1000.0);
		...
		TestCase.assertEquals(-0.172, Math.round(pair.getInput().getData(11)*1000.0)/1000.0);
		TestCase.assertEquals(0.014, Math.round(pair.getInput().getData(12)*1000.0)/1000.0);
		TestCase.assertEquals(0.0090, Math.round(pair.getInput().getData(13)*1000.0)/1000.0);
		TestCase.assertEquals(-0.062, Math.round(pair.getInput().getData(14)*1000.0)/1000.0);
		TestCase.assertEquals(0.066, Math.round(pair.getInput().getData(15)*1000.0)/1000.0);		
